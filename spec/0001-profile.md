...
This document describes the structure and semantics of the `Profiles.yaml` file, as well as the certificate policy enforced by the values it defines.
A `Profiles.yaml` file declares at the beginning which settings apply globally to all profiles.
After these global settings, the file can contain one or multiple profiles, each of which can configure settings related to cryptographic operations supported by the Crypto Broker.

The first part of this document describes the global settings and the structure of a profile.
The second part describes the certificate policy, i.e. how the values from the profile, the Certificate Signing Request, the CA certificate and additional API parameters map onto the attributes of the certificate generated by the `SignCertificate` API.
In general, the generated certificate must be a valid X.509 certificate and comply with [RFC 5280 Section 4.1](https://www.rfc-editor.org/rfc/rfc5280#section-4.1).
